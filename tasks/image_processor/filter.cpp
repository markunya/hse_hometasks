        ...
        for (size_t j = 0; j < image.Width(); ++j) {
            for (int64_t y = -static_cast<int>(matrix.size()) / 2; y < static_cast<int64_t>(matrix.size()) / 2 + 1;
                ...
                for (int64_t x = -static_cast<int>(matrix[0].size()) / 2;
                    ...
                    size_t image_y =
                        std::max(zero, std::min(static_cast<int64_t>(image.Height()) - 1, static_cast<int64_t>(i) + y));
                    size_t image_x =
                        std::max(zero, std::min(static_cast<int64_t>(image.Width()) - 1, static_cast<int64_t>(j) + x));
